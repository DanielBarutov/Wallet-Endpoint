...
def get_wallets(db: Session = Depends(get_db)):
    ...
    if wallet != None:
        result = [WalletRead(wallet_uuid=wallet.wallet_uuid, balance=wallet.balance) for wallet in wallet]
        return result
    else:
...
def get_wallet_balance(wallet_uuid: str, db: Session = Depends(get_db)):
    wallet = db.execute(select(Wallet).where(Wallet.wallet_uuid == wallet_uuid)).scalar()
    if wallet != None:
